...
releases: "0.22.0": {
	...
	description: """
		...
		| datadog_agent_remap_blackhole_acks    | 20.48MiB |      44.54 | 100.00%      |
		...
		| datadog_agent_remap_datadog_logs_acks | 9.55MiB  |      15.42 | 100.00%      |
		...
		| socket_to_socket_blackhole            | -4.61MiB |     -15.96 | 100.00%      |
		"""
	...
	changelog: [
		...
		},
		{
			...
			scopes: ["buffers"]
			description: """
				Vector now automatically migrates disk buffers from `v1` to `v2` as described in [Switching the default
				implementation of disk buffers to `disk_v2`](/highlights/2022-04-06-disk-buffer-v2-stable). In
				a future release, support for `v1` disk buffers will be dropped.
				"""
			pr_numbers: [12069]
		},
		{
		...
		},
		{
			...
			description: """
				VRL now emits a warning whenever it encounters an `if` condition that is static as this indicates
				a condition that can likely be removed.
				"""
			pr_numbers: [12396]
		},
		{
		...
		},
		{
			...
			scopes: ["kubernetes_logs source"]
			description: """
				The `kubernetes_logs` source now reads events in order whenever a pod log file rotates. Previously Vector could start reading the new file before it finished processing the previous one, resulting in the logs being out-of-order.
				"""
			pr_numbers: [12330]
		...
		},
		{
			...
			scopes: ["gcp provider"]
			description: """
				GCP sinks now correctly handle authentication token refreshing from the metadata service.
				"""
			pr_numbers: [12645]
		},
		{
